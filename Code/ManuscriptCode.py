# -*- coding: utf-8 -*-
"""clusteringManuscript_blankCode_updated260410.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1P9aTdW3bGFR-0SG1EoACJlMYIcQ-Frjx
...
## 2.Note
Download raster(s) of greenness extracted from Google Earth Engine. This should be in the user's Google Drive.
...
file1 = 'NM_studyArea_Greenness_file1.tif' #INPUT: file that contains the greenness raster 1 from GEE
file2 = 'NM_studyArea_Greenness_file2.tif' #INPUT: file that contains the greenness raster 2 from GEE
...
## Create plots of all the mean cluster trajectories
...
for i in np.arange(0,rows):
    for j in np.arange(0,cols):
        ...
        if cluster_number == num_clusters:
            break
#plt.savefig("NM_kmeans_clusterMeans.png") #INPUT: file name for figure of mean cluster time series
...
for i in np.arange(0,rows):
    for j in np.arange(0,cols):
        ...
        if cluster_number == num_clusters:
            break
#plt.savefig("NM_kmeans_clusterStdev.png") #INPUT: file name for plot of cluster mean time series and their standard deviations

"""## 3.5 Map the pixels back into space with the cluster assignment"""

##save to CSV
geoDF_reset.to_csv('NM_clustered_kmeans.csv', index = False) #INPUT: file name to save the time series plus their cluster assignment
...
outputFile = "NM_OR_clustered_kmeans.tif" #INPUT: file name for raster of clusters
with rio.open(
...
clusterMeans.to_csv("NM_kmeans_clusterMeans.csv") #INPUT: file name for CSV of mean cluster time series
...
clusterMeans_coords_gdf.to_file("NM_kmeans_clusterMeans_landtrendrShapefile.shp") #INPUT: file name to save shapefile with mean cluster time series
...
coords_gdf.to_file("NM_kmeans_clusterMeans_landtrendrCoords.shp") #INPUT: file name to save adjusted shapefile with just coordinates for mean cluster time series

"""### 4.Note.1
**(a)** Upload the shapefile with the mean cluster time series ("NM_kmeans_clusterMeans_landtrendrShapefile.shp") to Google Drive \\
**(b)** Upload the shapefile with just the coordinates of the mean cluster time series grid cells ("NM_kmeans_clusterMeans_landtrendrCoords.shp") as an asset in Google Earth Engine ([Manage assets](https://developers.google.com/earth-engine/guides/manage_assets)).
...
## set up aoi
aoi_fc = ee.FeatureCollection('projects/ee-project/assets/NM_kmeans_clusterMeans_landtrendrCoords') #INPUT: name of Google Earth Engine cloud project and path to file
...
data = gpd.read_file("/content/drive/MyDrive/NM_kmeans_clusterMeans_landtrendrShapefile.shp") #INPUT: file name for shapefile of mean cluster time series
data.head()

## change magnitude of data so that decimals don't get lost
scale = 1e4 ## this scaling number may need to be adjusted based on values of data
data[data.columns[0:-3]] = data[data.columns[0:-3]]*scale
...
for band in np.arange(1, bands.shape[0]+1):
    ...
    cmeans_LT.append(vals/scale) ## the values were scaled by a scaling number (1e4 here) -> unscale here

## create dataframe
cmeans_LT_df = pd.DataFrame(cmeans_LT, index = np.arange(start_year,end_year+1))
cmeans_LT_df_T = cmeans_LT_df.transpose()

## Create plots of all the mean cluster trajectories overlaid by LT
num_clusters = k_clusters
#colors = ["#eae200", "#7fccf6","#2bd78d","#510068","#f4a03c","#ff8bf2","#3da3a5","#d1b9e7","#b744eb","#8b86fc","#b1f892","#859c00","#5f5f5f","#007092"] #INPUT: set a color palette with colors for each cluster if desired; example given here for 14 clusters
## this sets up a set of subplots that should contain enough boxes for all of the clusters
rows = int(math.sqrt(num_clusters))
...
for i in np.arange(0,rows):
    for j in np.arange(0,cols):
        ...
        if cluster_number == num_clusters:
            break
#plt.savefig("NM_OR_kmeans_clusterMeans_LandTrendr.png") #INPUT: file name for plot of landtrendr time series overlaid on mean cluster time series
